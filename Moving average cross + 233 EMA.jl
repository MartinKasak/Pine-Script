//@version=4
strategy(title="Moving average cross+233EMA", shorttitle="MA cross+233EMA", calc_on_every_tick = true,overlay=true, process_orders_on_close=true, default_qty_type=strategy.percent_of_equity, default_qty_value=100)

movingaverage1 =input(10, title="fast moving average")
mma2002=sma(close, movingaverage1)
movingaverage2 =input(6, title="slow moving average")
...
plot(mma2004, title='very slow', color=#fff700, linewidth=1)
...
if testPeriod()
    strategy.entry("BuyOS", true, when = mma2002 > mma2003 and close > mma2004)
